...
def _try_gemini(file_bytes, file_type, extracted_text, prompt, model_name='gemini-3-flash-preview'):
    ...
    # Method One for PDF/Image Vision
    if any(t in file_type for t in ['png', 'webp', 'jpg', 'jpeg', 'image', 'pdf']):
        print(f"DEBUG: Gemini ({model_name}) using Method One for vision extraction - file_type: {file_type}")
        
        system_instruction = """You are an expert insurance policy analyzer. 
Extract the following information from the provided insurance policy PDF (could be text or scanned).
Ensure all amounts are extracted DIRECTLY as seen in the document, NOT calculated.
...
- TP (Third Party / Liability) Premium
- Net Premium
- Gross Premium"""

        mime_type = "application/pdf" if 'pdf' in file_type else "image/jpeg"
        if 'png' in file_type: mime_type = "image/png"
        elif 'webp' in file_type: mime_type = "image/webp"
        ...
        try:
            response = client.models.generate_content(
                ...
                contents=[
                    ...
                    "Extract the policy details in JSON format."
                ],
                config=types.GenerateContentConfig(
                    ...
                    response_schema={
                        ...
                        "properties": {
                            ...
                            "tp_premium": {"type": "STRING"},
                            "net_premium": {"type": "STRING"},
                            "gross_premium": {"type": "STRING"},
                            "status": {"type": "STRING", "enum": ["success", "failed"]}
                        },
                        "required": [
                            "policy_number", "insured_name", "vehicle_number", 
                            "policy_start_date", "policy_end_date", "od_premium", 
                            "tp_premium", "net_premium", "gross_premium", "status"
                        ]
                    },
                    temperature=0.1,
            ...
            print(f"DEBUG: Gemini Method One error: {str(e)}")
            # Fallback to existing text-based method if vision fails
            if 'pdf' in file_type:
                print("DEBUG: Falling back to text-based extraction for PDF...")
            else:
                raise e

    # Existing Text-Based Method (Fallback or for non-vision/non-PDF)
    print(f"DEBUG: Gemini ({model_name}) using text-based extraction...")
    ...
    try:
        ...
        print(f"DEBUG: Gemini text-based error: {str(e)}")
        raise e
